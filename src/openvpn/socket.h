                 ...
                 struct link_socket_actual *from)
{
    ...
    if (res > 0 && sock->scramble.enabled)
    {
        scramble_incoming(&sock->scramble, (uint8_t *)BPTR(buf), res);
    }
...
link_socket_write(struct link_socket *sock,
                  ...
                  struct link_socket_actual *to)
{
    if (sock->scramble.enabled)
    {
        scramble_outgoing(&sock->scramble, (uint8_t *)BPTR(buf), BLEN(buf));
    }
